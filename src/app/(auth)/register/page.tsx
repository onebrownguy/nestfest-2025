    ...
    try {
      const response = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({
          ...
          // Note: University, program, and graduation year fields are collected
          // but not stored yet - these would need additional table columns
        }),
      })
      ...
      if (response.ok) {
        ...
        if (result.error && result.details?.feedback) {
          // Password validation error
          const feedback = result.details.feedback
          showToast.error(`Password Requirements: ${feedback.warning || 'Password is too weak'}`)
        } else {
          ...
          <Form form={form} onSubmit={onSubmit} className="mt-6 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <FormInput
                ...
                helperText="At least 8 characters with special characters (!@#$%^&*)"
